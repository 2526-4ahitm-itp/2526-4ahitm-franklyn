...
Capture each student's text-file changes during an exam as per-minute incremental diffs, stored diff-only and bound to the student session, so a teacher can reconstruct the file history (FSD §9, US-05).
