...
The exam lifecycle for franklyn: a Lehrer creating exams with mandatory fields and a unique PIN, students joining via Keycloak and PIN within the valid time window, single global sessions per student, the recording start/stop windows, exam start/end transitions, and exam hard-deletion (FSD §4.2, §4.3, §5.1, §6, §12.2).
