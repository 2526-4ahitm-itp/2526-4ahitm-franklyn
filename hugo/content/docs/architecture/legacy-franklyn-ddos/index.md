...
to repeat the problem and analyze the server while it's running.
...
-   `noise`: how much noise the image contains (0 – 1)
    -   this effects png compressions time (the more noise the more time it takes)
-   `prefix`: text added before each random client name that shows up in the instructor webui

A local run showed the tester is limited solely by bandwidth at arround 2 GiB/s from a single laptop,
so the only real bottleneck is bandwidth—meaning it’s fully capable of stressing the Franklyn server
...
-   The client sends a full frame everytime instead of sending delta frames.
...
This required 18 requests per seconds but the server can only handle 16-17 max.
...
the server.
This will lead to a **~90+%** preformance gain as image encoding currently
