...
| **Verification** | `checksums.txt` required; an **exact entry for our asset** required (awk guard so `--ignore-missing` cannot silently pass a missing line); then `sha256sum --strict --check`. Fail closed on missing file / missing entry / mismatch. Never "install anyway." | `verify_checksum` |
